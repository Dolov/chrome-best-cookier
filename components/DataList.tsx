...
const DataList: React.FC<DataListProps> = (props) => {
  ...
  React.useEffect(() => {
    ...
    const indeterminate =
      data.some((item) => !item.checked) && data.some((item) => item.checked)
  ...
  const onChange = async (changedValues, cookie) => {
    if (changedValues.expirationDate) {
      changedValues.expirationDate =
        dayjs(changedValues.expirationDate).valueOf() / 1000
    }
    if (cookie.create) {
        ...
        <tr className="rounded-box bg-base-300">
          <td className="text-center align-middle">
            <input
              ...
              checked={checked}
              onChange={onCheckAll}
              className="checkbox-primary checkbox checkbox-sm mt-[5px]"
            />
          </td>
          <td>NO.</td>
          <th className="z-10 bg-base-300 text-center">
            ...
            <div className="center">
              ...
              <HeaderDomain
                ...
                setDomainList={(domainList) =>
                  setConditions({ ...conditions, domainList })
                }
              />
            </div>
          </td>
      ...
      <tbody>
        {cookies.map((cookie, index) => {
          ...
          return (
            <tr
              key={`${id}-${index}`}
              className={classnames("group hover:bg-base-200", {
              ...
              <td>
                <Input
                  ...
                  onChange={(value, callback) =>
                    onDomainChange(value, cookie, callback)
                  }
                />
              </td>
              <td className="ellipsis w-[150px]">
                <DatePicker
                  ...
                  onChange={(expirationDate) =>
                    onChange({ expirationDate }, cookie)
                  }
                />
              </td>
              <td>
              ...
              <td>
                <SameSite
                  value={sameSite}
                  onChange={(sameSite) => onChange({ sameSite }, cookie)}
                />
              </td>
              <td>
                <BooleanDisplay className="badge-primary" value={session} />
              </td>
              <td>{!create && <RowActions init={init} data={cookie} />}</td>
            </tr>
          )
        })}
      </tbody>
